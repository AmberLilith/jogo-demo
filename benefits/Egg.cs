...
private async void OnBodyEntered(Node2D body)
{
	if (body is Player player)
	{
		...
		// 1. Pegamos a Label
		Label scoreLabel = GetTree().Root.GetNode<Label>("ForestStage/HUD/HBoxContainer/MarginContainerScore/Score");
		...
		Vector2 targetStagePos = GetViewport().GetCanvasTransform().AffineInverse() * screenPos;

		// 3. O Tween agora usa a posição calculada do mundo
		var flyTween = CreateTween().SetParallel(true);
		
		flyTween.TweenProperty(this, "global_position", targetStagePos, 0.6f)
